...
module.exports = {
  createPayableVoucher: async (req, res) => {
    const { head_of_account, particular, cheque_no, challan_no, amount, type } =
      req.body;

    if (!head_of_account || !amount) {
      return res.status(400).json({
        message: "Both Head Of Account and Amount are required",
      });
    }
    ...
    return res.status(200).send({
      ...
      message: "Payable Voucher created succesfully",
    });
  },
};
