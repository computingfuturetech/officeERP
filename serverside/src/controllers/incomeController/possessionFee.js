...
module.exports = {
  createPossessionFee: async (req, res) => {
    ...
    try {
      ...
      const member = await MemberList.findOne({
        $expr: { $eq: [{ $toString: "$msNo" }, `${member_no}`] },
      });
      if (!member) {
    ...
    try {
      ...
      if (req.body.member_no) {
        const member = await Member.findOne({
          $expr: { $eq: [{ $toString: "$msNo" }, `${req.body.member_no}`] },
        });
        if (!member) {
          return res.status(404).json({ message: "Member not found" });
        }
  ...
  getPossessionFeeIncome: async (req, res) => {
    ...
    try {
      ...
      if (search) {
        let member = await MemberList.findOne({
      ...
      if (id) {
        possessionFeeIncome = await PossessionFee.findById(id)
          ...
          .exec();
        if (possessionFeeIncome) {
          possessionFeeIncome = await transformPaymentDetails(
            possessionFeeIncome
          );
        }
      } else {
      ...
      if (
        !possessionFeeIncome ||
        (Array.isArray(possessionFeeIncome) && possessionFeeIncome.length === 0)
      ) {
        return res.status(404).json({ message: "Possession Fee not found" });
      }
      if (member_no) {
