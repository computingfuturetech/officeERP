...
module.exports = {
  getAllExpense: async (req, res) => {
    const { expense_type, search, page = 1, limit = 10 } = req.query;
    ...
    try {
      ...
      const mainHeadAccounts = await MainHeadOfAccount.find({
      ...
      const subHeadAccounts = await SubExpenseHeadOfAccount.find({
      ...
      const expenses = await Promise.all([
        AuditFeeExpense.find(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .sort({ createdAt: -1 })
          .exec(),
        BankChargesExpense.find(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .sort({ createdAt: -1 })
          .exec(),
        ElectricityAndWaterConnectionExpense.find(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .sort({ createdAt: -1 })
          .exec(),
        LegalProfessionalExpense.find(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .sort({ createdAt: -1 })
          .exec(),
        MiscellaneousExpense.find(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .sort({ createdAt: -1 })
          .exec(),
        OfficeExpense.find(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .sort({ createdAt: -1 })
          .exec(),
        OfficeUtilExpense.find(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .sort({ createdAt: -1 })
          .exec(),
      ...
      if (search) {
        filteredExpenses = allExpenses.filter((expense) => {
          const headOfAccount = expense.mainHeadOfAccount
            ? expense.mainHeadOfAccount.headOfAccount
            : expense.subHeadOfAccount
            ? expense.subHeadOfAccount.headOfAccount
            : "";
  ...
  getExpenseByHeadOfAccount: async (req, res) => {
    ...
    if (!headOfAccountId || !mainId) {
      return res
        .status(400)
        .json({ message: "Head of Account ID and Main ID are required" });
    }
    ...
    try {
      ...
      const expenses = await Promise.all([
        AuditFeeExpense.findOne(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .populate("bank", "bankName accountNo")
          .exec(),
        BankChargesExpense.findOne(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .populate("bank", "bankName accountNo")
          .exec(),
        ElectricityAndWaterConnectionExpense.findOne(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .exec(),
        LegalProfessionalExpense.findOne(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .populate("bank", "bankName accountNo")
          .exec(),
        MiscellaneousExpense.findOne(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .populate("bank", "bankName accountNo")
          .exec(),
        OfficeExpense.findOne(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .populate("bank", "bankName accountNo")
          .exec(),
        OfficeUtilExpense.findOne(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .populate("bank", "bankName accountNo")
          .exec(),
        Salaries.findOne(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .populate("bank", "bankName accountNo")
          .exec(),
        VechicleDisposal.findOne(query)
          .populate("mainHeadOfAccount", "headOfAccount")
          .populate("subHeadOfAccount", "headOfAccount")
          .exec(),
      ]);
      ...
      if (!expense) {
        return res.status(404).json({
          message:
            "No expense found for the given head of account ID and main ID",
        });
      }
